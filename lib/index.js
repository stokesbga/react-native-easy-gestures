...
export default class Gestures extends Component {
  ...
  static propTypes = {
    ...
    outOfBoundsStyle: PropTypes.object,
    // Styles
  ...
  _getNextLocation = (event) => {
    ...
    const { containerLayout, statusBarHeight } = this.props
    ...
    if(boundLeft < containerLayout.x) {
      ...
      oob.x = 'max'
      oob.snapToPosition = { ...stp, left: containerLayout.width-width }
    }
    ...
    if(boundTop < containerLayout.y) {
      ...
      oob.y = 'max'
      oob.snapToPosition = { ...stp, ...oob.snapToPosition, top: containerLayout.height-height }
    }
  ...
  onDrag = (event, gestureState) => {
    ...
    const { draggable, topZindex, outOfBoundsStyle } = this.props;
    ...
    if((draggable || draggable.y) && !!oob) {
      // Out of bounds. Show indicator
      dragStyles = {
        ...dragStyles,
        ...outOfBoundsStyle
      }
    } else if((draggable || draggable.y) && !oob) {
      dragStyles = {
        ...dragStyles,
        borderWidth: 0,
      }
